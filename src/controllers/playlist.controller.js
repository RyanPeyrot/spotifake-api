...
exports.getAll = async (req, res) => {
    try {
        const Playlists = await Playlist.find().populate({ path: 'medias', populate: { path: 'artist' } });
...
exports.getPlaylistById = async (req, res) => {
    try {
        ...
        await Playlist.findById(PlaylistId).populate({ path: 'medias', populate: { path: 'artist' } }).then((doc) => {
            if (doc) {
...
exports.getByName = async (req,res) => {
    try{
        await Playlist.findOne({name:req.headers.name}).populate({ path: 'medias', populate: { path: 'artist' } }).then((doc) => {
            if (doc) {
                return res.status(200).json(doc)
            } else {
                return res.status(404).json({message: 'Aucun média trouvé'})
